...
无参数运行时，登录成功后会依次询问任务执行方式、播放速度和视频处理方式：

- 单任务：一次只处理一个章节。
- 多任务：输入同时处理的章节数，直接回车默认使用 4 个章节。
- 倍速播放：选择启用后输入 1.0 到 2.0 之间的播放速度；不启用时使用原速 1.0。
- 正常模式：只处理未完成视频，并从服务器记录的历史进度续播。
- 全部重看：忽略视频的已完成状态和历史进度，识别所有视频并从 0 秒完整观看；文档、作业等其他已完成任务不会重复处理。

对于要求累计有效观看时长的课程，建议选择单任务、原速和全部重看。程序可以保证重新顺序上报所有视频，但最终累计时长仍以学习通服务器记录为准。

使用配置文件或命令行参数时不会弹出这些选择，可用于无人值守运行：`jobs = 1` 为单任务，`jobs >= 2` 为多任务；`speed = 1` 为原速，`speed > 1` 为倍速；`video_mode = normal` 为正常模式，`video_mode = replay_all` 为全部重看模式。
...
```bash
python main.py -u 手机号 -p 密码 -l 课程ID1,课程ID2,课程ID3...(可选) -j 1 -s 1.5 --video-mode [normal|replay_all] -a [retry|ask|continue](可选)
```

其中 `-j 1` 表示单任务，`-j 2` 及以上表示多任务；`-s 1` 为原速，`-s <1.0-2.0>` 可设置播放倍速；`--video-mode replay_all` 表示所有视频从头完整观看，默认值 `normal` 保持正常续播逻辑。
